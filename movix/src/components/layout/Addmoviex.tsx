...
export default function AddMovieDialog({ onAdded }: { onAdded: () => void }) {
  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      ...
      <Dialog.Portal>
        {/* แยก Dialog.Overlay ออกมาเป็น self-closing tag */}
        <Dialog.Overlay className="bg-black/70 fixed inset-0 backdrop-blur-sm" />
        <Dialog.Content
          className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 
          w-[90%] max-w-2xl bg-white/10 backdrop-blur-md border border-white/20 
               text-white rounded-xl shadow-xl p-6 space-y-4"
        >
